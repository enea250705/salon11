...
export async function sendTimeOffApprovalNotification(user: User, type: string, startDate: string, endDate: string, duration?: string, startTime?: string, endTime?: string): Promise<boolean> {
  ...
  let durationText = 'giornata intera';
  if (duration === 'specific_hours' && startTime && endTime) {
    durationText = `orario specifico: ${startTime} - ${endTime}`;
  }
  ...
  const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
        ...
        <p>Gentile ${user.name},</p>
        <p>Ti informiamo che la tua richiesta di <strong>${typeTranslated}</strong> per il periodo <strong>${formattedStartDate} - ${formattedEndDate}</strong> (${durationText}) è stata <span style="color: green;"><strong>approvata</strong></span>.</p>
...
export async function sendTimeOffRejectionNotification(user: User, type: string, startDate: string, endDate: string, duration?: string, startTime?: string, endTime?: string): Promise<boolean> {
  ...
  const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
        ...
        <p>Gentile ${user.name},</p>
        <p>Ti informiamo che la tua richiesta di <strong>${typeTranslated}</strong> per il periodo <strong>${formattedStartDate} - ${formattedEndDate}</strong> (${durationText}) è stata <span style="color: red;"><strong>rifiutata</strong></span>.</p>
