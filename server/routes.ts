...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  passport.use(
    new LocalStrategy(async (username, password, done) => {
      try {
        console.log(`🔒 Tentativo di login con username/email: ${username}`);
        
        // Prima cerca per username
        let user = await storage.getUserByUsername(username);
        console.log(`🔍 Utente trovato tramite username: ${!!user}`);
        
        // Se non trovato, controlla se è una email
        if (!user && username.includes('@')) {
          console.log(`📧 Tentativo di ricerca per email: ${username}`);
          // Cerchiamo per email
          user = await storage.getUserByEmail(username);
          console.log(`📧 Utente trovato tramite email: ${!!user}`);
        }
        
        if (!user) {
          console.log(`❌ Nessun utente trovato per: ${username}`);
          return done(null, false, { message: "Credenziali non valide" });
        }
        
        console.log(`🔑 Verifica password per: ${user.username}`);
        if (user.password !== password) {
          console.log(`❌ Password non valida per: ${user.username}`);
          return done(null, false, { message: "Credenziali non valide" });
        }
        
        if (!user.isActive) {
          console.log(`❌ Account disabilitato: ${user.username}`);
  ...
  // Endpoint per ritirare un orario dalla pubblicazione
  app.post("/api/schedules/:id/unpublish", isAdmin, async (req, res) => {
    try {
      ...
      // Verifica se lo schedule è già non pubblicato
      if (!existingSchedule.isPublished) {
        console.log("Schedule is already unpublished:", scheduleId);
        return res.status(400).json({ message: "Schedule is already unpublished" });
      }
      
      // Aggiorna lo stato di pubblicazione dello schedule
      ...
      if (!updatedSchedule) {
        return res.status(500).json({ message: "Failed to unpublish schedule" });
      }
      
      console.log("Schedule unpublished successfully:", updatedSchedule);
      ...
      for (const user of users) {
        if (user.isActive && user.role !== 'admin') { // Invia solo ai dipendenti, non agli admin
          ...
          await storage.createNotification({
            userId: user.id,
            type: "schedule_update",
            message: "Pianificazione turni ritirata dalla pubblicazione",
            isRead: false,
            data: {
      ...
      res.status(500).json({ message: "Failed to unpublish schedule", error: String(err) });
    }
  });
              ...
              <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
                ...
                <div style="background-color: #f9f9f9; border-radius: 5px; padding: 15px; margin: 20px 0;">
                  ...
                  <ul style="margin-bottom: 0;">
                    ...
                    <li><strong>Durata:</strong> ${request.duration === 'full_day' ? 'Giornata intera' : request.duration === 'specific_hours' && request.startTime && request.endTime ? `Orario specifico: ${request.startTime} - ${request.endTime}` : request.duration === 'morning' ? 'Mattina' : 'Pomeriggio'}</li>
