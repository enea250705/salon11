...
import { generateTimeOffResponseEmail } from "@/lib/notification-service";
...
export function TimeOffApproval() {
  ...
  const approveRequest = useMutation({
    ...
    onSuccess: (_, requestId) => {
      ...
      if (user && request) {
        const emailData = generateTimeOffResponseEmail(
          user.email,
          user.name,
        ...
        apiRequest("POST", "/api/notifications", {
          ...
          message: `La tua richiesta di ${getTypeLabel(request.type)} è stata approvata`,
          data: {
    ...
    onSuccess: (_, requestId) => {
      ...
      if (user && request) {
        ...
        apiRequest("POST", "/api/notifications", {
          userId: user.id,
          type: "request_response",
          message: `La tua richiesta di ${getTypeLabel(request.type)} è stata respinta`,
          data: {
            requestId: request.id,
            type: request.type,
            startDate: request.startDate,
